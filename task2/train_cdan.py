"""
task2/train_cdan.py
Trains ResNet-18 with Conditional Domain Adversarial Network (CDAN).
Complies strictly with manual specifications:
- Seed: 6304
- Multilinear conditioning: g(x) = vec(f (x) p) -> dimension 512 * 7 = 3584
- Discriminator: Linear(3584 -> 256) -> ReLU -> Dropout(0.5) -> Linear(256 -> 2)
- Same dynamic schedule alpha(p) = 2 / (1 + exp(-10*p)) - 1 and unit loss weight as DANN
- Frozen BatchNorm running stats policy
- No entropy conditioning and NO detaching of f or p
- AdamW (lr=1e-4, weight_decay=1e-4)
- 70/30 stratified domain separability diagnostic with seed 6304
"""
...
# macOS File Descriptor Limit Fix to prevent [Errno 24]
try:
    import resource
...
from task2.models.adaptation_modules import GradientReverseLayer, DomainDiscriminator
...
def multilinear_conditioning(features: torch.Tensor, softmax_probs: torch.Tensor) -> torch.Tensor:
    ...
    Explicitly keeps gradients intact (NO detaching f or p as mandated by manual).
    """
    batch_size = features.size(0)
    # [B, 512, 1] * [B, 1, 7] -> [B, 512, 7]
...
class CDANModel(nn.Module):
    def __init__(self, pretrained: bool = True, num_classes: int = 7):
        ...
        self.grl = GradientReverseLayer(alpha=0.0)
        # Input dimension: 512 features * 7 classes = 3584
        self.discriminator = DomainDiscriminator(in_features=512 * num_classes, hidden_dim=256)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Multilinear tensor conditioning g(x) = vec(f (x) p)
        g = multilinear_conditioning(features, probs)
        reversed_g = self.grl(g)
        domain_logits = self.discriminator(reversed_g)

        return features, logits, domain_logits
...
def train_cdan(
    ...
    print("Architecture: CDAN (Multilinear vec(f (x) p) -> 3584 -> 256 -> 2)")
    ...
    model = CDANModel(pretrained=True, num_classes=7).to(device)
    ...
    best_model_path = os.path.join(save_dir, "cdan_best.pth")
    history_file = os.path.join(save_dir, "cdan_history.json")

    total_steps = max_epochs * len(train_iterator)
    global_step = 0
    ...
    print(f"\n--- Starting CDAN Training (Max Epochs={max_epochs}, Patience={patience}) ---")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for x_s, y_s, x_t in train_iterator:
            ...
            # Standard dynamic schedule: alpha(p) = 2 / (1 + exp(-10*p)) - 1
            p = float(global_step) / float(total_steps)
            current_alpha = 2.0 / (1.0 + np.exp(-10.0 * p)) - 1.0
            model.set_grl_alpha(current_alpha)
            global_step += 1

            optimizer.zero_grad()

            # Forward pass: Source
            _, logits_s, d_logits_s = model(x_s)
            loss_cls = criterion_cls(logits_s, y_s)

            # Forward pass: Target (unlabeled, only contributes to domain loss)
            _, _, d_logits_t = model(x_t)

            d_labels_s = torch.zeros(batch_size_s, dtype=torch.long, device=device)
            d_labels_t = torch.ones(batch_size_t, dtype=torch.long, device=device)

            loss_d_s = criterion_domain(d_logits_s, d_labels_s)
            loss_d_t = criterion_domain(d_logits_t, d_labels_t)
            loss_domain = 0.5 * (loss_d_s + loss_d_t)

            # Unit weight combination per manual
            total_loss = loss_cls + loss_domain
            total_loss.backward()
            optimizer.step()

            running_cls_loss += loss_cls.item() * batch_size_s
            running_domain_loss += loss_domain.item() * (batch_size_s + batch_size_t)
            running_total_loss += total_loss.item() * batch_size_s
            total_samples += batch_size_s

        epoch_cls_loss = running_cls_loss / total_samples
        epoch_domain_loss = running_domain_loss / (total_samples * 2)
        ...
        history["alpha"].append(float(current_alpha))
        ...
        print(
            ...
            f"Loss(Cls: {epoch_cls_loss:.4f} | Dom: {epoch_domain_loss:.4f} | a: {current_alpha:.2f}) | "
    ...
    print(f"\n--- CDAN Training Finished in {total_time:.2f} minutes ---")
    ...
    if os.path.exists(baseline_path):
        with open(baseline_path, "r") as f:
            b_data = json.load(f)
            baseline_acc = b_data.get("target_acc", 0.0)
    ...
    metrics_file = os.path.join(save_dir, "cdan_metrics.json")
    with open(metrics_file, "w") as f:
        json.dump(final_results, f, indent=2)

    print("\n" + "=" * 55)
    print("                   CDAN RESULTS TABLE")
    print("=" * 55)
