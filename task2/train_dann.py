"""
task2/train_dann.py
Trains ResNet-18 using Domain-Adversarial Neural Network (DANN).
Complies with manual:
- Seed 6304, AdamW (lr=1e-4, wd=1e-4), frozen BN
- 256-unit hidden layer discriminator, ReLU, Dropout(0.5), 2-class output
- Dynamic schedule alpha(p) = 2 / (1 + exp(-10*p)) - 1
- Unit weight loss combination
- 70/30 domain separability diagnostic
"""
...
class DANNModel(nn.Module):
    def __init__(self, pretrained: bool = True, num_classes: int = 7):
        ...
        self.discriminator = DomainDiscriminator(in_features=self.backbone.feature_dim, hidden_dim=256)
...
def train_dann(
    ...
    model = DANNModel(pretrained=True, num_classes=7).to(device)
    ...
    best_model_path = os.path.join(save_dir, "dann_best.pth")
    history_file = os.path.join(save_dir, "dann_history.json")
    ...
    print(f"\n--- Starting DANN Training (Max Epochs={max_epochs}, Patience={patience}) ---")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for x_s, y_s, x_t in train_iterator:
            ...
            # Standard DANN dynamic schedule: alpha(p) = 2 / (1 + exp(-10*p)) - 1
    ...
    print(f"\n--- DANN Training Finished in {total_time:.2f} minutes ---")
    ...
    metrics_file = os.path.join(save_dir, "dann_metrics.json")
    with open(metrics_file, "w") as f:
        json.dump(final_results, f, indent=2)

    print("\n" + "=" * 55)
    print("                   DANN RESULTS TABLE")
    print("=" * 55)
